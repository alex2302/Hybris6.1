...
public class DefaultSapOaaCommerceStockService extends DefaultCommerceStockService implements SapOaaCommerceStockService
{
			...
			final PointOfServiceModel pointOfServiceModel) throws ATPException
	{
		...
		if (productAvailabilityList != null && !productAvailabilityList.isEmpty())
		{
			availableStock = atpAggregationStrategy.aggregateAvailability(cartGuid, productModel, pointOfServiceModel,
					productAvailabilityList).longValue();
		}
	...
	public List<ATPProductAvailability> getAvailabilityForProducts(final String cartGuid, final String itemId,
			final String productUnit, final List<ProductModel> productList) throws ATPException
	{
			...
			final BaseStoreModel baseStore)
	{
		...
		try
		{
			...
			for (final ATPProductAvailability availability : availabilities)
			{
				...
				if (getAtpAggregationStrategy().aggregateAvailability(null, product, pointOfService, availability.getAvailabilityList()).longValue() > 0 && pointOfService != null)
				{
	 ...
	 * 
	 * @see
	 * com.sap.retail.oaa.commerce.services.stock.SapOaaCommerceStockService#getAvailableStockLevelForPos(java.lang.String
			...
			final PointOfServiceModel pointOfService) throws ATPException
	{
		...
		if (productAvailabilityList != null && !productAvailabilityList.isEmpty())
		{
			availableStock = atpAggregationStrategy.aggregateAvailability(null, productModel, pointOfService,
					productAvailabilityList).longValue();
		}
