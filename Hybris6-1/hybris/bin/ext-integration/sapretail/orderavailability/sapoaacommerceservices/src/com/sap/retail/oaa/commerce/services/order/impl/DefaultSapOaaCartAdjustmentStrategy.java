...
public class DefaultSapOaaCartAdjustmentStrategy implements SapOaaCartAdjustmentStrategy
{
			...
			final long quantityToAdd, final long cartItemQty, final PointOfServiceModel pointOfServiceModel)
	{
		...
		// In case the actual cart item will be removed, we can skip the AvailableStockLevel check
		if (newTotalQuantity == 0)
		{
			return new Long(-cartItemQty);
		}
		...
		try
		{
			stockLevel = oaaStockService.getAvailableStockLevel(cartModel.getGuid(), itemId, productModel, pointOfServiceModel)
					.longValue();
		}
		...
		if (maxOrderQuantity != null)
		{
			...
			return new Long(newTotalQuantityAfterProductMaxOrder - cartItemQty);
		}
		return new Long(newTotalQuantityAfterStockLimit - cartItemQty);
	}
