...
public class DefaultSapOaaCartValidationStrategyTest
{
	...
	private static final Integer CART_ITEM_ENTRY_NO_1 = new Integer(0);
	private static final Integer CART_ITEM_ENTRY_NO_2 = new Integer(1);
	private static final Long CART_ITEM_QTY_1 = new Long(10);
	private static final Long CART_ITEM_QTY_2 = new Long(10);
	...
	public void validateCartSourcingFailsTest()
	{
		...
		classUnderTest.cleanCart(cartModel);
		EasyMock.expectLastCall();
		EasyMock.expect(classUnderTest.checkAvailabilityOfCartItems(EasyMock.anyObject(CartModel.class))).andReturn(
				new ArrayList<CommerceCartModification>());
	...
	public void validateCartEntryNoProductServiceTest()
	{
		...
		EasyMock.expect(productServiceMock.getProductForCode(EasyMock.anyObject(String.class))).andThrow(
				new UnknownIdentifierException("Test Exception"));
		...
		EasyMock
				.expect(
						cartServiceMock.getEntriesForProduct(EasyMock.anyObject(CartModel.class),
								EasyMock.anyObject(ProductModel.class))).andReturn(entriesList).anyTimes();
	...
	public void checkAvailabilityOfCartItemsShipToAndPOSNoStockTest()
	{
		...
		EasyMock
				.expect(
						classUnderTest.validateCartEntry(EasyMock.anyObject(CartModel.class),
								EasyMock.anyObject(AbstractOrderEntryModel.class), EasyMock.anyLong())).andReturn(noStockAvailableStatus)
				.anyTimes();
		...
		EasyMock
				.expect(
						stockServiceMock.getAvailabilityForProduct(EasyMock.anyObject(String.class), EasyMock.anyObject(String.class),
								EasyMock.anyObject(ProductModel.class))).andReturn(availabilityList).anyTimes();
		EasyMock
				.expect(
						stockServiceMock.getAvailabilityForProductAndSource(EasyMock.anyObject(String.class),
	...
	public void checkAvailabilityOfCartItemsExceptionTest()
	{
		...
		EasyMock
				.expect(
						stockServiceMock.getAvailabilityForProduct(EasyMock.anyObject(String.class), EasyMock.anyObject(String.class),
								EasyMock.anyObject(ProductModel.class))).andThrow(new ATPException()).anyTimes();
		EasyMock
				.expect(
						stockServiceMock.getAvailabilityForProductAndSource(EasyMock.anyObject(String.class),
								EasyMock.anyObject(String.class), EasyMock.anyObject(ProductModel.class),
								EasyMock.anyObject(String.class))).andThrow(new ATPException()).anyTimes();
		EasyMock.replay(stockServiceMock);
		classUnderTest.setOaaStockService(stockServiceMock);
