...
public class DefaultATPServiceTest
{
	...
	public void setup() throws SourcingException, URISyntaxException
	{
		...
		// Mock DefaultRestInitializationStrategy
		...
		classUnderTest.setTargetUrl(TARGET_URL);
		classUnderTest.setSapCarClient(CAR_CLIENT);
		classUnderTest.setOaaProfile(OAA_PROFILE);
	}

	@Test
	public void callRestAvailabilityServiceWithoutOaaProfile()
	{
		classUnderTest.setOaaProfile(null);
		List<ATPAvailability> atpAvailModel = null;

		//ATP Product Service
		try
		{
			atpAvailModel = classUnderTest.callRestAvailabilityServiceForProduct(null, null, new ProductModel());

			Assert.fail("ATPException expected because no OAA Profile is set");
		}
		...
		try
		{
			atpAvailModel = classUnderTest.callRestAvailabilityServiceForProductAndSource(new ProductModel(), "0001");
