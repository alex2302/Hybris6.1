...
public class CsticValueModelImpl extends BaseModelImpl implements CsticValueModel
{
	...
	public boolean equals(final Object obj)
	{
		...
		else
		{
			...
			{
				return equalsNumericalValues(other);
			}
		}
		return true;
	}

	protected boolean equalsNumericalValues(final CsticValueModelImpl other)
	{
		boolean isEqual = true;
		final Pattern numericFormatPattern = numericFormatCache.get();
		if (numericFormatPattern.matcher(name).matches() && numericFormatPattern.matcher(other.name).matches())
		{
			if (0 != Double.compare(Double.parseDouble(name), Double.parseDouble(other.name)))
			{
				isEqual = false;
			}
		}
