...
import de.hybris.platform.b2bacceleratoraddon.jalo.actions.AcceptQuoteAndOrderAction;
...
public abstract class GeneratedB2bacceleratoraddonManager extends Extension
{
	...
	public AcceptQuoteAndOrderAction createAcceptQuoteAndOrderAction(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( B2bacceleratoraddonConstants.TC.ACCEPTQUOTEANDORDERACTION );
			return (AcceptQuoteAndOrderAction)type.newInstance( ctx, attributeValues );
		}
		...
		{
			throw new JaloSystemException( e ,"error creating AcceptQuoteAndOrderAction : "+e.getMessage(), 0 );
		}
	}
	
	public AcceptQuoteAndOrderAction createAcceptQuoteAndOrderAction(final Map attributeValues)
	{
		return createAcceptQuoteAndOrderAction( getSession().getSessionContext(), attributeValues );
	}
